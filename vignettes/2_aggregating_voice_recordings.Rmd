...
```
### This markdown shows an example of aggregating raw sensor data (e.g., low level descriptors of voice extracted from mobile recordings) to some higher level aggregates. 
...
There are the following functions for data

(A) Illustrative dataset about data collected from one patient:
get_sample_one_patient_psychiatric_data()
get_sample_one_patient_sensor_data_recordings()
get_sample_one_patient_sensor_data_chunks()

(B) Illustrative synthetic dataset about data collected from more patients:
get_sample_psychiatric_data()
get_sample_sensor_data()

Read the documentation for more information.
In this vignette, we will illustrate the preprocessing of one patient data (A).


```{r}
visits <- get_sample_one_patient_psychiatric_data()
mobile_recordings <- get_sample_one_patient_sensor_data_recordings()
mobile_chunks <- get_sample_one_patient_sensor_data_chunks()
```
...
We calculated statistics like this (only first 16 rows showed here):
